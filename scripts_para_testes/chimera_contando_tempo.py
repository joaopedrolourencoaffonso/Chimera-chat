...
def telethon_module(fim, lock, my_id):
    ...
    from time import timezone, perf_counter
                    ...
                    for mensagem in mensagens:
                        if mensagem[2] == 2:
                            ...
                            if isfile(mensagem[3]):
                                x1 = perf_counter();#momento de início da trasferência de arquivo
                                msg = await client.send_file(mensagem[1], mensagem[3]);
                                x2 = perf_counter();#momento de término da trasferência de arquivo
                                print("Tempo de envio de arquivo: ", x2 - x1)
        ...
        try:
            ...
            if not event.is_channel:
                ...
                if hasattr(event.media, "document"):
                    ...
                    path = abspath(path);
                    x2 = perf_counter();#momento de término da trasferência de arquivo
                    print("Tempo de recebimento de arquivo: ", x2 - x1)
